...
TriggerPhrase = str(args["phrase"]).split(sep=None, maxsplit=-1)
...
for i in range(0, len(results["text"])):
    if int(results["conf"][i]) > args["min_conf"]:           # confidence filter

        # if statement for debug purpose
        if args["detail_img"] is not None:
...
if args["detail_img"] is not None:
    cv2.imwrite(args["detail_img"], image)
